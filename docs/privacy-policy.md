...
- **On demand, one word at a time:** Only for a word that has **no bundled English definition**, and only when you click the **K kheng.info** button or press the **K** key, KhmerLens sends **that single Khmer word** to kheng.info to fetch its definition, which is then shown inline (and can fill an Anki card).
...
Requested **only if you enable the "Look up missing definitions on kheng.info" feature** in the options page. It lets KhmerLens fetch a definition from kheng.info for a word that has no bundled English definition. A request is made only when you click the **K kheng.info** button or press **K**, and it contains **only that single Khmer word** — no page content, browsing history, or personal data. Results are cached briefly on your device. If you never enable the feature, this permission is never requested.
